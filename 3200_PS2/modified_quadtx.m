function [Q,fcount] = modified_quadtx(F,a,b,tol,varargin)
...
if abs(Q2 - Q1) <= tol
   ...
   %Put a limit on tol * 0.5 so there is not a stack overflow / runtime error.
   if(tol * .5 > 1.e-25)
       [Qa,ka] = quadtxstep(F, a, c, tol * .5, fa, fd, fc, varargin{:});
       [Qb,kb] = quadtxstep(F, c, b, tol * .5, fc, fe, fb, varargin{:});
   else
